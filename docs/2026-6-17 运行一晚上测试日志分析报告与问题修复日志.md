# 2026-6-17 运行一晚上测试日志分析报告与问题修复日志
...
$$
\text{throughput}=\frac{\text{block\_bytes}}{\text{write\_ms}/1000}
$$
...
\frac{33{,}600{,}000}{0.4414}\approx76.1\ \text{MB/s}
$$
...
\frac{33{,}600{,}000}{0.1856}\approx181.0\ \text{MB/s}
$$
...
$$
\text{remaining\_seconds}=\frac{\text{free\_bytes}}{\text{bytes\_per\_second}}
$$
...
\frac{100 \times 1024^3}{33{,}600{,}000}\approx3196\ \text{s}\approx53.3\ \text{min}
$$
...
本次已按日志分析结论修复 P1、P2、保存统计口径和慢写/慢查/慢读统计口径问题，修改范围如下：

| 修复项 | 修改文件 | 修复内容 | 预期效果 |
| --- | --- | --- | --- |
| P1 写盘耗时抖动 | `src/data_saver.py` | 写入路径从强制 `tobytes()` 调整为优先使用 `memoryview(data).cast('B')`；新增写盘 `p50/p95/p99/max`、慢写次数和连续慢写统计；慢写告警从固定 50 ms 改为按吞吐与硬阈值判断 | 减少 32 MiB 大块拷贝开销，降低低价值 warning 噪声，保留真正低吞吐写盘事件 |
| P2 `query_buffer_points()` 偶发阻塞 | `src/pcie6921_api.py`、`src/acquisition_thread.py` | API 层单次慢查询阈值调整为 100 ms；采集线程不再重复记录每次慢查询，只记录连续慢查询；新增 `query_p95_ms/query_p99_ms/query_slow_count` | 避免 API 层和采集线程对同一事件重复报警，保留 DLL 单次阻塞和连续阻塞两类诊断信息 |
| 慢读统计 | `src/acquisition_thread.py` | 新增 `read_p95_ms/read_p99_ms/read_max_ms/read_slow_count`；读数据超过 `max(50 ms, min(200 ms, block_duration_ms * 0.25))` 时记录慢读 warning | 能捕捉类似本次 `read_ms=304.6` 的尖峰，并记录块大小、缓冲点数和分位数上下文 |
| 保存统计日志准确性 | `src/data_saver.py` | 停止日志改为区分 `Current file bytes` 与 `Total bytes`；`FrameBasedFileSaver.stop()` 增加幂等保护；总统计改为单独 `Frame saver summary` | 避免 `Bytes written: 0` 被误解为整轮未保存，减少重复停止日志 |
| GUI 周期诊断快照 | `src/main_window.py` | 周期快照追加 `query_p95/query_p99/query_slow/read_p95/read_p99/read_slow/save_write_p95/save_write_p99/save_write_slow/save_total_bytes` | 夜间测试日志可直接观察长期分位数和累计慢事件，不必离线二次解析 |
| 单元测试 | `tests/test_data_saver.py` | 新增按帧切分保存器测试，验证跨文件总字节数、停止幂等和诊断字段 | 防止保存统计口径回退 |
...
但 warning 触发条件不再是固定 `write_ms > 50`，而是：

- 写盘吞吐低于 150 MiB/s；或
- 单次写盘耗时大于等于 500 ms；
- 触发后只记录前 3 次、每 10 次、或保存队列非空时的关键事件。

慢查询现在分两层记录：

- API 层：`query_buffer_points()` 单次 DLL 调用大于等于 100 ms 时记录 warning。
- 采集线程层：只在连续 3 次及之后每 10 次慢查询时记录 `Repeated slow query_buffer_points`。

慢读阈值随采集块周期缩放：

$$
\text{read\_warn\_ms}=\max(50,\min(200,\text{block\_duration\_ms}\times0.25))
$$

### 修复后建议复测

1. 使用本次相同参数运行 30 分钟以上，确认 `[ERROR]=0`、`save_dropped=0`、`gui_skips=0`。
2. 观察日志中的 `save_write_p95/save_write_p99/save_write_slow`，判断写盘是否仍有长期低吞吐。
3. 观察 `query_p95/query_p99/query_slow`，确认慢查询 warning 数量相比原日志明显下降且仍保留阻塞事件。
4. 观察 `read_p95/read_p99/read_slow`，确认慢读尖峰会以 warning 形式记录上下文。
5. 停止采集后确认日志不再出现容易误解的 `Bytes written: 0`，而是显示 `Current file bytes` 与 `Total bytes`。

## 8. 修复后结论

硬盘存满之前的原始日志没有显示数据丢失，但暴露出可观测性和统计口径不足。本次修复没有改变采集参数模型和 DLL 调用顺序，重点增强保存线程、采集线程和 GUI 周期快照的诊断能力。后续夜间测试应以 `save_write_p95/save_write_p99`、`query_p95/query_p99`、`read_p95/read_p99` 和 `save_total_bytes` 作为主要判断依据。
